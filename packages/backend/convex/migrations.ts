  ...
  handler: async (ctx) => {
    ...
    for (const user of users) {
      if (!user.subscription) {
        ...
        if (sub.messagesUsed === undefined || !Number.isFinite(sub.messagesUsed)) {
          updates.messagesUsed = 0;
          needsUpdate = true;
        }
        if (sub.messagesLimit === undefined || !Number.isFinite(sub.messagesLimit)) {
          updates.messagesLimit = 50;
          needsUpdate = true;
        }
        if (sub.premiumMessagesUsed === undefined || !Number.isFinite(sub.premiumMessagesUsed)) {
          updates.premiumMessagesUsed = 0;
          needsUpdate = true;
        }
        if (sub.premiumMessagesLimit === undefined || !Number.isFinite(sub.premiumMessagesLimit)) {
        ...
        if (sub.planPriceSol === undefined || !Number.isFinite(sub.planPriceSol)) {
          updates.planPriceSol = 0;
          needsUpdate = true;
        }
        if (sub.tokensUsed === undefined || !Number.isFinite(sub.tokensUsed)) {
          updates.tokensUsed = 0;
          needsUpdate = true;
        }
        if (sub.tokensLimit === undefined || !Number.isFinite(sub.tokensLimit)) {
          updates.tokensLimit = 10000;
          needsUpdate = true;
        }
