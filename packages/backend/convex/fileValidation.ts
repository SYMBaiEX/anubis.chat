...
export async function validateFile(
  ...
  try {
    ...
    const mimeValidation = validateMimeType(declaredMimeType || '', extension, bytes);
    ...
    const contentCheck = await checkDangerousContent(bytes, declaredMimeType || '');
...
function validateMimeType(
  ...
  if (typeConfig.magic && bytes.length >= 4) {
    ...
    if (!magicBytes.startsWith(typeConfig.magic)) {
      return {
        valid: false,
        error: 'File content doesn\'t match declared type (magic bytes mismatch)',
      };
    }
  }
...
function checkCompressionBomb(
  ...
  if (bytes[0] === 0x50 && bytes[1] === 0x4B) { // PK header (ZIP)
