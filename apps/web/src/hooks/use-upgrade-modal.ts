...
export function useUpgradeModal(options: UseUpgradeModalOptions = {}) {
  ...
  const [suggestedTier, setSuggestedTier] = useState<'pro' | 'pro_plus'>(defaultTier);
  ...
  const showForFeatureRequest = useCallback((feature: 'api_access' | 'large_files' | 'advanced_agents') => {
    // API access and large files require Pro+
    const tier = ['api_access', 'large_files'].includes(feature) ? 'pro_plus' : 'pro';
  ...
  const shouldAutoShow = autoShow && upgradePrompt?.shouldShow && upgradePrompt.urgency === 'high';
