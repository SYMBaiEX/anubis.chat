...
export function PaymentIntegrationTest() {
  ...
  const runAuthenticationTests = useCallback(async () => {
    ...
    updateTestResult(suiteName, 'auth_status', { status: 'pending', message: 'Checking authentication...' });
    
    if (!auth.isAuthenticated || !auth.user) {
      updateTestResult(suiteName, 'auth_status', {
        status: 'fail',
        message: 'User not authenticated. Please connect and authenticate your wallet.'
      });
      return false;
    }
    ...
    updateTestResult(suiteName, 'wallet_consistency', { status: 'pending', message: 'Checking wallet consistency...' });
    ...
    if (auth.user.walletAddress !== publicKey?.toBase58()) {
      updateTestResult(suiteName, 'wallet_consistency', {
        ...
        details: { auth: auth.user.walletAddress, wallet: publicKey?.toBase58() }
      });
      return false;
    }
    ...
    updateTestResult(suiteName, 'profile_data', { status: 'pending', message: 'Validating user profile...' });
    
    const requiredFields = ['walletAddress', 'isActive', 'subscription'];
    const missingFields = requiredFields.filter(field => !auth.user[field as keyof typeof auth.user]);
  ...
  const runSubscriptionTests = useCallback(async () => {
    ...
    updateTestResult(suiteName, 'subscription_status', { status: 'pending', message: 'Fetching subscription status...' });
    ...
    updateTestResult(suiteName, 'usage_limits', { status: 'pending', message: 'Calculating usage limits...' });
    
    const expectedCanSend = subscription.messagesUsed < subscription.messagesLimit;
    const expectedCanPremium = subscription.tier !== 'free' && 
      subscription.premiumMessagesUsed < subscription.premiumMessagesLimit;
    
    if (limits.canSendMessage !== expectedCanSend || limits.canUsePremiumModel !== expectedCanPremium) {
      updateTestResult(suiteName, 'usage_limits', {
        status: 'fail',
        message: 'Usage limits calculation mismatch',
        details: {
          expected: { canSend: expectedCanSend, canPremium: expectedCanPremium },
          actual: { canSend: limits.canSendMessage, canPremium: limits.canUsePremiumModel }
        }
      });
      return false;
    }
    ...
    updateTestResult(suiteName, 'feature_gating', { status: 'pending', message: 'Validating feature access...' });
    ...
    const featureMismatches = Object.keys(expectedFeatures).filter(
      key => expectedFeatures[key as keyof typeof expectedFeatures] !== 
  ...
  const runModelAccessTests = useCallback(async () => {
    ...
    updateTestResult(suiteName, 'standard_model', { status: 'pending', message: 'Testing standard model access...' });
    ...
    updateTestResult(suiteName, 'premium_model', { status: 'pending', message: 'Testing premium model access...' });
    ...
    const expectedAccess = subscription?.tier !== 'free' && 
      (subscription?.premiumMessagesUsed || 0) < (subscription?.premiumMessagesLimit || 0);
  ...
  const runUsageTrackingTests = useCallback(async () => {
    ...
    updateTestResult(suiteName, 'message_tracking', { status: 'pending', message: 'Testing message usage tracking...' });
    ...
    updateTestResult(suiteName, 'premium_tracking', { status: 'pending', message: 'Testing premium usage tracking...' });
  ...
  const runPaymentTests = useCallback(async () => {
    ...
    updateTestResult(suiteName, 'payment_validation', { status: 'pending', message: 'Testing payment validation...' });
    ...
    updateTestResult(suiteName, 'payment_api', { status: 'pending', message: 'Testing payment API endpoint...' });
  ...
  const runSecurityTests = useCallback(async () => {
    ...
    updateTestResult(suiteName, 'unauthorized_access', { status: 'pending', message: 'Testing unauthorized access protection...' });
    ...
    updateTestResult(suiteName, 'amount_validation', { status: 'pending', message: 'Testing payment amount validation...' });
    ...
    updateTestResult(suiteName, 'env_config', { status: 'pending', message: 'Checking environment configuration...' });
  ...
  const runAllTests = useCallback(async () => {
    setIsRunning(true);
    setTestSuites([
      { name: 'Authentication', description: 'User authentication and wallet integration', tests: [] },
      { name: 'Subscription', description: 'Subscription status and limits', tests: [] },
      { name: 'Model Access', description: 'AI model access controls', tests: [] },
      { name: 'Usage Tracking', description: 'Message and premium usage tracking', tests: [] },
      { name: 'Payment Processing', description: 'Solana payment processing', tests: [] },
      { name: 'Security', description: 'Security and validation tests', tests: [] }
    ]);
  ...
  const getStatusBadge = (status: TestResult['status']) => {
    switch (status) {
      ...
      case 'error':
        return <Badge variant="secondary" className="bg-orange-500 text-white">Error</Badge>;
      case 'pending':
      default:
          ...
          <div className="flex flex-col gap-4">
            ...
            {auth.isAuthenticated && (
              <Alert>
                ...
                <AlertDescription>
                  Authenticated as {auth.user?.walletAddress?.slice(0, 8)}...{auth.user?.walletAddress?.slice(-8)}
                ...
                {suite.tests.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {suite.tests.map((test) => (
                      <div key={test.name} className="flex items-start justify-between p-3 border rounded">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="p-3 border rounded">
                      ...
                      <div className="font-semibold">{subscription.messagesUsed} / {subscription.messagesLimit}</div>
                    </div>
                    <div className="p-3 border rounded">
                      <div className="text-sm text-muted-foreground">Premium Used</div>
                      <div className="font-semibold">{subscription.premiumMessagesUsed} / {subscription.premiumMessagesLimit}</div>
                    </div>
                    <div className="p-3 border rounded">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { label: 'Can Send Messages', value: limits.canSendMessage, remaining: limits.messagesRemaining },
                  { label: 'Can Use Premium Models', value: limits.canUsePremiumModel, remaining: limits.premiumMessagesRemaining },
                  { label: 'Can Upload Large Files', value: limits.canUploadLargeFiles },
                  { label: 'Advanced Features Access', value: limits.canAccessAdvancedFeatures },
                  { label: 'API Access', value: limits.canUseAPI },
                ].map((item) => (
                  <div key={item.label} className="p-3 border rounded flex items-center justify-between">
                    ...
                    <div className="space-y-2">
                      ...
                      {upgradePrompt.suggestedTier && (
                        <div className="text-sm">
                          Suggested upgrade: <Badge>{upgradePrompt.suggestedTier.toUpperCase()}</Badge>
                          {' '}(Urgency: <Badge variant={upgradePrompt.urgency === 'high' ? 'destructive' : upgradePrompt.urgency === 'medium' ? 'secondary' : 'default'}>
