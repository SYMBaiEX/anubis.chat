  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>Connected: <Badge variant={wallet.isConnected ? "default" : "secondary"}>{wallet.isConnected ? "Yes" : "No"}</Badge></div>
            <div>Connecting: <Badge variant={wallet.isConnecting ? "default" : "secondary"}>{wallet.isConnecting ? "Yes" : "No"}</Badge></div>
            <div>Authenticating: <Badge variant={wallet.isAuthenticating ? "default" : "secondary"}>{wallet.isAuthenticating ? "Yes" : "No"}</Badge></div>
            <div>Health Score: <Badge variant={wallet.connectionHealthScore > 60 ? "default" : "destructive"}>{wallet.connectionHealthScore}</Badge></div>
          </div>
          
          {wallet.publicKey && (
            <div className="text-sm">
              <strong>Public Key:</strong> <code className="text-xs">{wallet.formatAddress(8)}</code>
            </div>
          )}
        ...
        <CardContent className="space-y-2">
          ...
          {auth.user && (
            <div className="space-y-1 text-sm">
              <div><strong>User ID:</strong> <code className="text-xs">{auth.user.walletAddress}</code></div>
              {auth.user.role && <div><strong>Role:</strong> {auth.user.role}</div>}
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>Initialized: <Badge variant={solanaAgent.isInitialized ? "default" : "secondary"}>{solanaAgent.isInitialized ? "Yes" : "No"}</Badge></div>
            <div>Agents Available: <Badge variant="secondary">{solanaAgent.agents?.length || 0}</Badge></div>
            <div>Selected Agent: <Badge variant={solanaAgent.selectedAgent ? "default" : "secondary"}>{solanaAgent.selectedAgent?.name || "None"}</Badge></div>
            <div>Pending Transactions: <Badge variant="secondary">{solanaAgent.pendingTransactions.length}</Badge></div>
          </div>
        ...
        <CardContent>
          <div className="space-y-2">
            {[
              ...
              { key: 'solana_agent_init', label: 'Solana Agent Initialization' },
              { key: 'agent_balance_consistency', label: 'Agent Balance Consistency' },
            ].map(({ key, label }) => (
