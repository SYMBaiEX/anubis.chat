...
const ReferralTrackingContext =
  createContext<ReferralTrackingContextValue | null>(null);
...
export function ReferralTrackingProvider({
  children,
}: ReferralTrackingProviderProps) {
  const searchParams = useSearchParams();
  const { getStoredReferralCode, clearStoredReferralCode, trackReferralCode } =
    useReferralTracking();
    ...
    if (refParam) {
      ...
      if (isValidCode) {
        trackReferralCode(refParam.toUpperCase()).then((success) => {
          if (success) {
            log.info('Referral code tracked successfully', {
              referralCode: refParam.toUpperCase(),
            });
          } else {
            log.warn('Failed to track referral code', {
              referralCode: refParam.toUpperCase(),
            });
          }
        });
...
export function useReferralTrackingContext(): ReferralTrackingContextValue {
  ...
  if (!context) {
    throw new Error(
      'useReferralTrackingContext must be used within a ReferralTrackingProvider'
    );
  }
  return context;
}
