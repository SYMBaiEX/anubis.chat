...
const generateObjectSchema = z.object({
  ...
  schema: z.enum(['tasks', 'meetingSummary', 'articleSummary', 'contactInfo', 'jsonStructure']).optional(),
  ...
  (data) => data.schema || data.customSchema,
  {
...
function jsonSchemaPropertyToZod(prop: JSONSchemaProperty): z.ZodTypeAny {
  switch (prop.type) {
    case 'string':
      ...
      if (prop.enum) {
        const stringEnums = prop.enum.filter((v): v is string => typeof v === 'string');
    ...
    case 'number':
      ...
      if (prop.enum) {
        const numberEnums = prop.enum.filter((v): v is number => typeof v === 'number');
        if (numberEnums.length > 0) {
          // For number enums, use literal union instead of enum
          return z.union(numberEnums.map(n => z.literal(n)) as [z.ZodLiteral<number>, ...z.ZodLiteral<number>[]]);
        }
      }
      return numberSchema;
...
function getSchema(schemaType?: string, customSchema?: Record<string, unknown>): z.ZodTypeAny {
  ...
  return aiRateLimit(request, async (req) => {
    return withAuth(req, async (authReq: AuthenticatedRequest) => {
      try {
        ...
        console.log(`Object generation requested: ${generationId} by ${walletAddress} using ${model}, schema: ${schemaType || 'custom'}`);
        
        // Generate structured object
        const result = await generateObject({
          model: aiModel,
          system: systemPrompt || 'Generate a structured object based on the prompt and schema provided.',
          prompt,
          schema,
        ...
        console.log(`Object generated: ${generationId}, tokens: ${result.usage.totalTokens}`);
