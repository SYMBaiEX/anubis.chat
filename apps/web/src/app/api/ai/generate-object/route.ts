  ...
  return aiRateLimit(request, async (req) => {
    return withAuth(req, async (authReq: AuthenticatedRequest) => {
      try {
        ...
        log.error('AI object generation error', { error: error instanceof Error ? error.message : String(error) });
...
export async function GET(request: NextRequest) {
  return withAuth(request, async (authReq: AuthenticatedRequest) => {
    try {
      ...
      log.error('Get schemas error', { error: error instanceof Error ? error.message : String(error) });
      const response = NextResponse.json(
