...
const createChatSchema = z.object({
  ...
  systemPrompt: z.string().max(2000, 'System prompt must be 2000 characters or less').optional(),
  temperature: z.number().min(0).max(2).optional(),
  maxTokens: z.number().min(1).max(8000).optional(),
  initialMessage: z.string().max(10000, 'Initial message must be 10000 characters or less').optional(),
});
  ...
  return chatRateLimit(request, async (req) => {
    return withAuth(req, async (authReq: AuthenticatedRequest) => {
      try {
        ...
        // Mock chat data
        const mockChats: Chat[] = Array.from({ length: Math.min(limit, 5) }, (_, i) => ({
          _id: nanoid(12),
          walletAddress,
          title: search ? `Search: ${search} - Chat ${i + 1}` : `Chat ${i + 1}`,
        ...
        const nextCursor = hasMore ? mockChats[mockChats.length - 1]._id : undefined;
        
        console.log(`Listed ${mockChats.length} chats for user ${walletAddress}`);
