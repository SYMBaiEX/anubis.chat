 ...
 */
export function formatSpeechText(text: string, isInterim: boolean = false): string {
  if (!text) return '';
  ...
  processed = processed.replace(/\b(capital|capitalize|uppercase|caps|all caps)\s+(\w+)/gi, (match, command, word) => {
 ...
 */
export function getVoiceCommands(): { punctuation: string[], formatting: string[], special: string[] } {
  return {
    punctuation: PUNCTUATION_PHRASES.map(([phrase]) => phrase).filter((v, i, a) => a.indexOf(v) === i),
