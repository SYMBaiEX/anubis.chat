...
describe('solana utilities', () => {
  describe('formatSolanaAddress', () => {
    ...
    it('should handle edge cases for address formatting', () => {
      // Very long address
      const longAddress =
        'HN7cABqLq46Es1jh92dQQi5DKyXJxfogVQKMrNDK9HbBExtraLongAddressForTesting';
      ...
      expect(
        formatSolanaAddress('HN7cABqLq46Es1jh92dQQi5DKyXJxfogVQKMrNDK9HbB', 0)
      ).toBe('...');
      expect(
        formatSolanaAddress('HN7cABqLq46Es1jh92dQQi5DKyXJxfogVQKMrNDK9HbB', 20)
      ).toBe('HN7cABqLq46Es1jh92dQ...KyXJxfogVQKMrNDK9HbB');
    });
  });

  describe('lamportsToSol', () => {
    it('should convert lamports to SOL correctly', () => {
      expect(lamportsToSol(1_000_000_000)).toBe(1); // 1 SOL
      expect(lamportsToSol(500_000_000)).toBe(0.5); // 0.5 SOL
      expect(lamportsToSol(0)).toBe(0);
      expect(lamportsToSol(1)).toBe(0.000_000_001);
    });

    it('should handle large numbers', () => {
      expect(lamportsToSol(10_000_000_000_000)).toBe(10_000); // 10,000 SOL
    });

    it('should throw error for negative numbers', () => {
      expect(() => lamportsToSol(-1_000_000_000)).toThrow(
        'Invalid lamports value: must be a non-negative finite number'
      );
    });

    it('should throw error for non-finite numbers', () => {
      expect(() => lamportsToSol(Number.NaN)).toThrow(
        'Invalid lamports value: must be a non-negative finite number'
      );
      expect(() => lamportsToSol(Number.POSITIVE_INFINITY)).toThrow(
        'Invalid lamports value: must be a non-negative finite number'
      );
      expect(() => lamportsToSol(Number.NEGATIVE_INFINITY)).toThrow(
        'Invalid lamports value: must be a non-negative finite number'
      );
    });
  });
  ...
  describe('solToLamports', () => {
    it('should convert SOL to lamports correctly', () => {
      expect(solToLamports(1)).toBe(1_000_000_000); // 1 SOL
      expect(solToLamports(0.5)).toBe(500_000_000); // 0.5 SOL
      expect(solToLamports(0)).toBe(0);
      expect(solToLamports(0.000_000_001)).toBe(1);
    });

    it('should handle large numbers', () => {
      expect(solToLamports(10_000)).toBe(10_000_000_000_000);
    });

    it('should throw error for negative numbers', () => {
      expect(() => solToLamports(-1)).toThrow(
        'Invalid SOL value: must be a non-negative finite number'
      );
    });
    ...
    it('should throw error for non-finite numbers', () => {
      ...
      expect(() => solToLamports(Number.NEGATIVE_INFINITY)).toThrow(
        'Invalid SOL value: must be a non-negative finite number'
      );
    });
  });
  ...
  describe('validateSolanaAddress', () => {
    ...
    it('should reject invalid addresses', () => {
      ...
      expect(
        validateSolanaAddress('HN7cABqLq46Es1jh92dQQi5DKyXJxfogVQKMrNDK9HbBX')
      ).toBe(false); // Too long
    });
    ...
    it('should validate PublicKey objects', () => {
      const publicKey = new PublicKey(
        'HN7cABqLq46Es1jh92dQQi5DKyXJxfogVQKMrNDK9HbB'
      );
    ...
    it('should reject addresses with invalid characters', () => {
      expect(
        validateSolanaAddress('HN7cABqLq46Es1jh92dQQi5DKyXJxfogVQKMrNDK9Hb@')
      ).toBe(false);
      expect(
        validateSolanaAddress('HN7cABqLq46Es1jh92dQQi5DKyXJxfogVQKMrNDK9Hb#')
      ).toBe(false);
  ...
  describe('createSignInMessage', () => {
    ...
    it('should throw error for invalid public key', () => {
      expect(() => createSignInMessage('')).toThrow(
        'Invalid public key provided'
      );
      expect(() => createSignInMessage('invalid')).toThrow(
        'Invalid public key provided'
      );
      expect(() => createSignInMessage('123')).toThrow(
        'Invalid public key provided'
      );
    });
