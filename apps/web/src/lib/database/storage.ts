...
export interface StorageBackend {
  ...
  getUserDocuments(walletAddress: string, options?: {
    page?: number;
    limit?: number;
    category?: string;
    search?: string;
  ...
  canAccessDocument(walletAddress: string, documentId: string): Promise<boolean>;
  addDocumentToUser(walletAddress: string, documentId: string): Promise<void>;
  removeDocumentFromUser(walletAddress: string, documentId: string): Promise<void>;
  ...
  // Nonce operations
  storeNonce(publicKey: string, nonce: string, expiresAt: number): Promise<void>;
...
export class InMemoryStorage implements StorageBackend {
  ...
  async getUserDocuments(walletAddress: string, options: {
    page?: number;
    limit?: number;
    category?: string;
    search?: string;
  ...
  async canAccessDocument(walletAddress: string, documentId: string): Promise<boolean> {
    const document = this.documents.get(documentId);
    return document ? document.ownerId === walletAddress : false;
  }
  
  async addDocumentToUser(walletAddress: string, documentId: string): Promise<void> {
  ...
  async removeDocumentFromUser(walletAddress: string, documentId: string): Promise<void> {
...
export function createStorage(): StorageBackend {
  ...
  switch (storageType.toLowerCase()) {
    ...
    default:
      console.warn(`Unknown storage type '${storageType}', falling back to in-memory`);
      return new InMemoryStorage();
  }
}
